		...
		char *b, int sz)
{
    ...
    // Step 2: Search for an entry with the given xid in the client's reply list.
    auto list_it = std::find_if(rep_list.begin(), rep_list.end(),
                                [xid](const reply_t &rep) {
                                    return rep.xid == xid;
                                });

    // Step 3: Update the entry if found, or create a new one if not.
    if (list_it != rep_list.end()) {
...
{
    // Step 1: Remove entries with xid <= xid_rep from the client's reply list.
    ScopedLock rwl(&reply_window_m_);
    std::list<reply_t> &rep_list = reply_window_[clt_nonce];
    auto list_iter = rep_list.end();
    for (auto it = rep_list.begin(); it != rep_list.end();) {
        if (it->xid <= xid_rep) {
            free(it->buf);
            it = rep_list.erase(it);
        } else {
            // Check for an entry with the given xid in the client's reply list.
            if (it->xid == xid) list_iter = it;
            ++it;
        }
    }


    // Step 3: Determine the state of the request based on the search results.
    rpcstate_t ret;
    if (list_iter == rep_list.end()) {
        // Case 1: Entry not found - create a new entry and set the state to NEW.
        reply_t rep(xid);
        rep_list.push_back(rep);
        ret = NEW;
    } else if (list_iter->cb_present) {
        // Case 2: Entry found with callback present - set the state to DONE and return the buffer and size.
        *b = list_iter->buf;
        *sz = list_iter->sz;
        ret = DONE;
    } else if (xid > xid_rep) {
        // Case 3: Entry found without callback and xid > xid_rep - set the state to INPROGRESS.
        ret = INPROGRESS;
    } else {
        // Case 4: Entry found without callback and xid <= xid_rep - set the state to FORGOTTEN.
        ret = FORGOTTEN;
    }
